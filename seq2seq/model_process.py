...
def train(exp_name, unique_id,
          model, training_data, validation_data, interpolate_data, extrapolate_data, optimizer, device, epochs,
          interpolate_loader=None, tb=None, log_interval=100, interpolate_interval=1, extrapolate_interval=1,
  ...
  for epoch_i in range(start_epoch, epochs):
    ...
    train_accu_list.append(train_accu_list)
    ...
    if epoch_i % interpolate_interval == 0:
        ...
        print('[Interpolate]  loss: {interpolate_loss},  ppl: a{ppl: 8.5f}, accuracy: {accu:3.3f} %, '\
                'elapse: {elapse:3.3f}ms'.format(
    ...
    if epoch_i % extrapolate_interval == 0:
        start = time.time()
        extrapolate_loss, extrapolate_accu = extrapolate_epoch(model, extrapolate_data, device, epoch_i, tb, log_interval)
        print('[Extrapolate]  loss: {extrapolate_loss},  ppl: {ppl: 8.5f}, accuracy: {accu:3.3f} %, '\
                'elapse: {elapse:3.3f}ms'.format(
                    extrapolate_loss=extrapolate_loss, ppl=math.exp(min(extrapolate_loss, 100)), accu=100*extrapolate_accu,
                    elapse=(time.time()-start)*1000))
...
def extrapolate_epoch(model, extrapolate_data, device, epoch, tb=None, log_interval=100):
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(extrapolate_data, mininterval=2, leave=False)):
            # prepare data
            batch_qs, batch_qs_pos, batch_as, batch_as_pos = map(lambda x: x.to(device), batch)
            gold_as = batch_as[:, 1:]
